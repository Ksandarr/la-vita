...
const BookingSection = () => {
  const { t } = useLanguage()
  const { houses, complex, selectedHouseId } = useData()
  const [formData, setFormData] = useState({
    checkIn: '',
    checkOut: '',
    selectedVilla: houses[0]?.id || 'villa-bella',
    guests: '2',
    name: '',
    email: '',
    phone: '',
  ...
  // Update selectedVilla when selectedHouseId changes from villa selection
  useEffect(() => {
    if (selectedHouseId) {
      setFormData(prev => ({
        ...prev,
        selectedVilla: selectedHouseId
      }))
    }
  }, [selectedHouseId])
  ...
  const selectedVilla = houses.find(house => house.id === formData.selectedVilla) || houses[0]

  const nights = calculateNights()
  const pricePerNight = selectedVilla?.pricing?.basePrice || 950
...
{/*             </p> */}
            ...
            <div className="contact-info">
              <h3>{t('booking.contactInformation')}</h3>
              <div className="contact-details">
                <div className="contact-item">
                  ...
                  <span>{complex.contact.phone}</span>
                </div>
                <div className="contact-item">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span>{complex.contact.email}</span>
                </div>
                <div className="contact-item">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="booking-form-container">
            <form onSubmit={handleSubmit} className="booking-form">
              ...
              {/* Villa Selection */}
              <div className="form-group villa-selection">
                <label htmlFor="selectedVilla">{t('booking.selectHouse')}</label>
                <select
                  id="selectedVilla"
                  name="selectedVilla"
                  value={formData.selectedVilla}
                  onChange={handleInputChange}
                  required
                  className="villa-select"
                >
                  {houses.map((house) => (
                    <option key={house.id} value={house.id}>
                      {house.name} - {house.pricing.currency} {house.pricing.basePrice}/night ({house.capacity.guests} guests)
                    </option>
                  ))}
                </select>
                {selectedVilla && (
                  <div className="selected-villa-info">
                    <small>{selectedVilla.shortDescription}</small>
                  </div>
                )}
              </div>
              ...
              <div className="form-row">
                <div className="form-group">
                  <label htmlFor="name">{t('booking.firstName')} & {t('booking.lastName')}</label>
                  <input
                    type="text"
                    id="name"
                    name="name"
              ...
              {nights > 0 && (
                <div className="booking-summary">
                  <div className="summary-header">
                    <h4>{t('booking.bookingSummary')}</h4>
                  </div>
                  <div className="summary-row">
                    <span>{selectedVilla?.name}</span>
                    <span>{selectedVilla?.pricing?.currency} {pricePerNight}/{t('ui.night')}</span>
                  </div>
                  <div className="summary-row">
                    <span>{selectedVilla?.pricing?.currency} {pricePerNight} × {nights} {t('booking.nights')}</span>
                    <span>{selectedVilla?.pricing?.currency} {totalPrice}</span>
                  </div>
